...
Optional personal Google Drive sync is implemented in normal navigation using the private `appDataFolder` scope. It requires explicit consent, keeps refresh credentials in the device-only Keychain, keeps access tokens in memory, validates bounded backup-v8 snapshots, saves a local recovery snapshot before applying remote changes, and appends immutable idempotent backups. On the separate physical-iPhone QA installation, the owner completed Google consent and observed the app's **Backed up to Google Drive** result, establishing the live token and upload path. Independent remote read-back, disconnect/revocation, non-owner consent, and Android interoperability remain release gates. iCloud, live widget data sharing, and OCR are not implemented. Team Apple/Google sign-in remains behind inactive APIs and is separate from personal Drive sync. The widgets intentionally need no App Group and expose no financial data. Both native Files sheets have been opened on a physical iPhone, but a completed external Files save/import round trip, notification delivery, and share-extension transfer remain acceptance boundaries. Their status is documented without presenting unverified behavior as working.
